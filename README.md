...
A full reorderable grid implemention similar to Flutters Reorderable_List. with full `CombinableReorderableGridView`, `CombinableReorderableGrid` and `SliverCombinableReorderableGrid` implementations
...
`CombinableReorderableGridView` is a drop in replacement for the existing `GridView` and adds an `onReorder` callback that provides the original and new index of the reordered item.
...
return MaterialApp(
    home: Scaffold(
      body: CombinableReorderableGridView.extent(
...
`CombinableReorderableGrid` provides all the constructors and parameters the normal `GridView` has. The package also includes:

- `CombinableReorderableGridView`, which is a prebuild Material-ish implementation of the grid.
- `CombinableReorderableGrid`, A barebones widget that allows you to customize the grid however you want
- `SliverCombinableReorderableGrid`, a reorderable grid sliver for custom scroll implementations
